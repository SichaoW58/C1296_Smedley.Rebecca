...
tetrachoric<-tetrachoric(df_1 %>% select(-case_number,-location_self,-location_report,-tumor_thickness,-mitotic_count,-intact,-size, -beyond_dermis) %>% mutate(sex=ifelse(sex=="1",1,0)) %>%  mutate_all(as.character) %>% mutate_all(as.numeric))
...
#| label: cor-polychor
#| warning: false
polychoric<-polychoric(df_1 %>% select(Ki67,location_self,location_report,size,beyond_dermis) %>% mutate_all(as.character) %>% mutate_all(as.numeric))
...
No diagnostic stats because beyond_dermis is not binary.

```{r}
#| label: beyond_dermis_accuracy
#| eval: false

beyond_dermis_cellcount<-df_1_cells %>% count(beyond_dermis_cell)%>% drop_na() %>% bind_rows(data.frame(beyond_dermis_cell="FP",n=0))
